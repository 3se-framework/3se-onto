...
# Human-readable labels for evaluation relation fields rendered on term pages.
EVALUATION_RELATION_LABELS: dict[str, str] = {
    "evaluates": "Evaluates",
}
...
def build_hosted_by_index(terms: list[dict]) -> dict[str, list[dict]]:
    """
    Return a mapping of URI -> list of term entries that declare that URI
    as a hosts target. Used to compute the inverse 'hosted by'
    relation: if A hosts B, then B is hosted by A.
    """
    index: dict[str, list[dict]] = {}
    for term in terms:
        val = term.get("hosts")
...
def render_term_page(term: dict, ref_index: dict, superclass_index: dict | None = None,
    ...
    # Hosted by (computed inverse of hosts)
    if hosted_by_index:
        term_id = term.get("@id", "")
        hosting_terms = hosted_by_index.get(term_id, [])
        if hosting_terms:
            links = [render_uri_link(t.get("@id", "")) for t in hosting_terms]
            bfo_html += (
                f'<tr>'
                f'<td>Hosted by</td>'
                f'<td>{SEP.join(links)}</td>'
    ...
    # Flow/data relations (produces, consumes, conveys)
    flow_html = ""
    ...
    # Evaluation relations (evaluates) and its inverse (evaluated by)
    evaluation_html = ""
    for field, label in EVALUATION_RELATION_LABELS.items():
        val = term.get(field)
        if not val:
            continue
        uris = [val] if isinstance(val, str) else val
    ...
    if evaluated_by_index:
        ...
        if evaluating_terms:
            links = [render_uri_link(t.get("@id", "")) for t in evaluating_terms]
            evaluation_html += (
                f'<tr>'
                f'<td>Evaluated by</td>'
                f'<td>{SEP.join(links)}</td>'
                f'</tr>'
            )
